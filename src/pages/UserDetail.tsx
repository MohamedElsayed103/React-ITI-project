      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <p className="text-gray-600"><span className="font-semibold">Website:</span> {user?.website}</p>
              <p className="text-gray-600"><span className="font-semibold">Company:</span> {user?.company.name}</p>
              <p className="text-gray-600"><span className="font-semibold">City:</span> {user?.address.city}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="max-h-96 overflow-y-auto space-y-4">
              {posts?.map((post) => (
                <div key={post.id} className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition">
                  <h3 className="font-semibold text-lg text-gray-800 capitalize">{post.title}</h3>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      />
                      <span className={isCompleted ? 'text-green-700' : 'text-gray-800'}>
                        {todo.title}
                      </span>
                    </div>
                  </div>
